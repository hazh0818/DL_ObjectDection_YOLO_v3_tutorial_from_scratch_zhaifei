"""
这个程序包含了predict_transform函数（Darknet类中的forward函数要用到），
write_results函数使我们的输出满足 
objectness 分数阈值和非极大值抑制（NMS），以得到「真实」检测结果。
还有prep_image和letterbox_image等图片预处理函数等
（前者用来将numpy数组转换成PyTorch需要的的输入格式，
后者用来将图片按照纵横比进行缩放，将空白部分用(128,128,128)填充）。
话不多说，直接看util.py的注释。
"""
...
def unique(tensor):
    # 因为同一类别可能会有多个真实检测结果，所以我们使用unique函数来去除重复的元素，
    # 即一类只留下一个元素，达到获取任意给定图像中存在的类别的目的。
...
def predict_transform(prediction, inp_dim, anchors, num_classes, CUDA = True):
    ...
    #比如有13个格子，刚x_y_offset的坐标就对应为(0,0),(0,1)…(12,12) .view(-1, 2)将tensor变成两列，
    # unsqueeze(0)在0维上添加了一维。
...
def write_results(prediction, confidence, num_classes, nms_conf = 0.4):
    ...
    for ind in range(batch_size):
        ...
        for cls in img_classes:
            ...
            # cls_mask本身为15x7，cls_mask[:,-2]将cls_mask的倒数第二列取出来，
            # 此时是1维tensor，torch.nonzero(cls_mask[:,-2])得到的是非零元素的索引，
            ...
            # 这里的sort()将返回两个tensor，
            # 第一个是每个框含有有目标的分数由低到高排列，
            # 第二个是现在由高到底的tensor中每个元素在原来的序号。
            # [0]是排序结果, [1]是排序结果的索引
            conf_sort_index = torch.sort(image_pred_class[:,4], descending = True )[1]
            #根据排序后的索引对应出的bbox的坐标与分数，依然为4x7的tensor
            image_pred_class = image_pred_class[conf_sort_index]
            idx = image_pred_class.size(0)   #detections的个数 Number of detections
            ...
            for i in range(idx):
                ...
                try:
                    # image_pred_class[i].unsqueeze(0)，为什么要加unsqueeze(0)？
                    # 这里image_pred_class为4x7的tensor，image_pred_class[i]是一个长度为7的tensor，
                ...
                # 而ious < nms_conf得到的是torch.uint8类型，用float()将它们转换为float类型。
                ...
                # Remove the non-zero entries 开始移除
                # torch.nonzero返回的是索引，是2维tensor。
                # 将经过iou_mask掩码后的每个方框含有目标的得分为非0的方框的索引提取出来，
...
def letterbox_image(img, inp_dim):
    ...
    w = inp_dim[0]
    h = inp_dim[1] #inp_dim是需要resize的尺寸（如416*416）
...
def prep_image(img, inp_dim): 
    ...
    因此，我们在 util.py 中写了一个函数 prep_image 
    用来将numpy数组转换成PyTorch需要的的输入格式。即（3，416,416）
    为神经网络准备输入图像数据
    返回值: 处理后图像, 原图, 原图尺寸
    """
    ...
    # from_numpy(()将ndarray数据转换为tensor格式，div(255.0)将每个元素除以255.0，
    # 进行归一化，unsqueeze(0)在0维上添加了一维，
...
def load_classes(namesfile):
    ...
    load_classes会返回一个字典——将每个类别的索引映射到其名称的字符串
    """
    print(namesfile)
    fp = open(namesfile, "rb")
    names = fp.read().decode('utf8').split("\n")[:-1]
    return names
